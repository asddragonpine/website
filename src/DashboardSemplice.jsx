...
// const ALL_COLUMNS = [
//   { id: 'numprog', label: 'N° Progressivo', defaultVisible: true, pinned: false },
...
//   { id: 'mondiali', label: 'Partecipazione mondiali', defaultVisible: true, pinned: false },
...
//       // Usa il numprog direttamente dal database
//       setIscrizioni(data || []);
//     } catch (err) {
...
//     return items.map(i => ({
//       'Numero Progressivo': i.numprog ? String(i.numprog).padStart(3, '0') : 'N/D',
//       'Nome': i.nome || '',
//       'Cognome': i.cognome || '',
//       'Email': i.email || '',
...
//       'Partecipazione mondiali': i.mondiali === true ? 'Sì' :
//                         i.mondiali === false ? 'No' : 'N/D',             
...
//       doc.setFontSize(10);
//       doc.text("C.F. 02420980225 – Via C. Battisti 50 – 38042 Baselga di Piné TN", 105, 40, { align: "center" });
...
//       y = addRow("Numero iscrizione:", selectedIscrizione.numprog 
//         ? String(selectedIscrizione.numprog).padStart(3, '0') : 'N/D', y);
...
//       const mondialiText = selectedIscrizione.mondiali === true ? 'Sì' :
//                          selectedIscrizione.mondiali === false ? 'No' : 'N/D';
//       y = addRow("Mondiali:", mondialiText, y);
...
//           switch(column) {
//             case 'numprog':
//               return iscrizione.numprog && 
//                      String(iscrizione.numprog).includes(filterValue);
...
//               case 'mondiali':
//               // Converti il valore mondiali in etichetta leggibile
//               const mondialiLabel = iscrizione.mondiali === true ? 'Sì' :
//                                  iscrizione.mondiali === false ? 'No' : 'N/D';
//               return mondialiLabel.toLowerCase().includes(filterValueLower);
...
//           case 'numprog':
//             return isAsc 
//               ? (a.numprog || 0) - (b.numprog || 0)
//               : (b.numprog || 0) - (a.numprog || 0);
...
//                   {iscrizione.numprog 
//                     ? `#${String(iscrizione.numprog).padStart(3, '0')} - ` 
...
//                       {visibleColumns.mondiali && (
//                         <Box>
//                           <Typography variant="caption" color="text.secondary" display="block">Partecipazione mondiali</Typography>
//                           <Typography variant="body2" sx={{ fontWeight: 500 }}>
//                             {iscrizione.mondiali === true ? 'Sì' :
//                              iscrizione.mondiali === false ? 'No' : 'N/D'}
//                           </Typography>
//                         </Box>
//                       )}
...
//                   N° {selectedIscrizione.numprog 
//                       ? String(selectedIscrizione.numprog).padStart(3, '0') 
...
//                       </TableRow>
//                       <TableRow>
//                         <TableCell component="th">Partecipazione mondiali:</TableCell>
//                         <TableCell>
//                           {selectedIscrizione.mondiali === true ? 'Sì' :
//                            selectedIscrizione.mondiali === false ? 'No' : 'N/D'}
//                         </TableCell>
//                       </TableRow>
//                       <TableRow>
...
const DashboardSemplice = () => {
  ...
  const [certificateFilterType, setCertificateFilterType] = useState('all'); // 'all', 'expired', 'expiring', 'expiringOrExpired'
  const [anchorElCertFilter, setAnchorElCertFilter] = useState(null);
  ...
  // Funzioni per certificati medici
  const isCertificateExpired = (dateString) => {
  ...
  const isCertificateExpiring = (dateString) => {
    if (!dateString) return false;
    
    const today = new Date();
    const expiryDate = new Date(dateString);
    
    // Considera "in scadenza" i certificati che scadono entro 30 giorni
  ...
  // Funzioni per filtro certificati
  const handleOpenCertFilter = (event) => {
    setAnchorElCertFilter(event.currentTarget);
  };
  
  const handleCloseCertFilter = () => {
    setAnchorElCertFilter(null);
  };
  
  const handleApplyCertFilter = (type) => {
    setCertificateFilterType(type);
    setShowExpiringCertificates(type !== 'all');
    handleCloseCertFilter();
  };
  ...
  const filteredIscrizioni = useMemo(() => {
    return iscrizioni
      .filter(iscrizione => {
        ...
        if (showExpiringCertificates) {
          if (certificateFilterType === 'expired') {
            if (!isCertificateExpired(iscrizione.dataScadenzaVisita)) return false;
          } else if (certificateFilterType === 'expiring') {
            if (!isCertificateExpiring(iscrizione.dataScadenzaVisita)) return false;
          } else if (certificateFilterType === 'expiringOrExpired') {
            if (!isCertificateExpired(iscrizione.dataScadenzaVisita) && 
                !isCertificateExpiring(iscrizione.dataScadenzaVisita)) return false;
          }
        }
  ...
  }, [iscrizioni, searchTerm, columnFilters, order, orderBy, showExpiringCertificates, certificateFilterType]);
        ...
        <Stack spacing={2}>
          ...
          {showExpiringCertificates && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              <Chip 
                size="small" 
                label={certificateFilterType === 'expired' ? 'Scaduti' : 
                       certificateFilterType === 'expiring' ? 'In scadenza' :
                       'Da verificare'}
                color={certificateFilterType === 'expired' ? 'error' : 'warning'}
                onDelete={() => {
                  setShowExpiringCertificates(false);
                  setCertificateFilterType('all');
                }}
          ...
          <Stack direction="row" spacing={1} justifyContent="space-between">
            ...
            <Button
              ...
              startIcon={<Close />}
              disabled={activeFiltersCount === 0 && !showExpiringCertificates}
            >
              Reset filtri
            </Button>
          </Stack>
                      ...
                      {visibleColumns.dataScadenzaVisita && (
                        <Box>
                          ...
                          <Typography variant="body2" sx={{ 
                            fontWeight: 500,
                            color: isCertificateExpired(iscrizione.dataScadenzaVisita) ? 'error.main' :
                                   isCertificateExpiring(iscrizione.dataScadenzaVisita) ? 'warning.main' : 'inherit',
                            display: 'flex',
                            alignItems: 'center'
                          }}>
                            {formatDate(iscrizione.dataScadenzaVisita)}
                            {isCertificateExpired(iscrizione.dataScadenzaVisita) && (
                            ...
                            {isCertificateExpiring(iscrizione.dataScadenzaVisita) && (
                              <Box component="span" sx={{ 
                                display: 'inline-block', 
                                width: 8, 
                                height: 8, 
                  ...
                  {visibleColumns.dataScadenzaVisita && (
                    <TableCell>
                      <Box sx={{ 
                        ...
                        {formatDate(iscrizione.dataScadenzaVisita)}
                        {isCertificateExpired(iscrizione.dataScadenzaVisita) && (
                          <Tooltip title="Certificato scaduto">
                            <Box component="span" sx={{ 
                              display: 'inline-block', 
                              width: 10, 
                              height: 10, 
                        ...
                        {isCertificateExpiring(iscrizione.dataScadenzaVisita) && (
                          <Tooltip title="Certificato in scadenza entro 30 giorni">
                            <Box component="span" sx={{ 
                              display: 'inline-block', 
                              width: 10, 
                              height: 10, 
    ...
    return (
      <Container maxWidth="lg" sx={{ py: 4 }}>
        <Paper elevation={3} sx={{ p: 3, mb: 4 }}>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mb: 3 }}>
            ...
            >
              ...
              {/* Pulsante filtro certificati */}
              <Button
                variant={showExpiringCertificates ? "contained" : "outlined"}
                color={showExpiringCertificates ? "secondary" : "default"}
                onClick={handleOpenCertFilter}
                startIcon={<Assignment />}
                size="small"
              >
                {showExpiringCertificates ? (
                  certificateFilterType === 'expired' ? 'Certificati Scaduti' :
                  certificateFilterType === 'expiring' ? 'Certificati in Scadenza' :
                  'Cert. da Verificare'
                ) : 'Certificati'}
              </Button>
            ...
            {(activeFiltersCount > 0 || showExpiringCertificates) && (
              <Button
                variant="outlined" 
                color="error"
                onClick={() => {
                ...
                fullWidth={isMobile}
              >
                Reset filtri ({activeFiltersCount + (showExpiringCertificates ? 1 : 0)})
              </Button>
            )}
          </Stack>
        ...
        {/* Menu per filtro certificati */}
        <Menu
          anchorEl={anchorElCertFilter}
          open={Boolean(anchorElCertFilter)}
          onClose={handleCloseCertFilter}
        >
          <MenuItem onClick={() => handleApplyCertFilter('all')}>
            Visualizza tutti
          </MenuItem>
          <MenuItem onClick={() => handleApplyCertFilter('expired')}>
            Certificati scaduti
          </MenuItem>
          <MenuItem onClick={() => handleApplyCertFilter('expiring')}>
            Certificati in scadenza (30 giorni)
          </MenuItem>
          <MenuItem onClick={() => handleApplyCertFilter('expiringOrExpired')}>
            Da verificare (scaduti o in scadenza)
          </MenuItem>
        </Menu>
                  ...
                  <Table size="small">
                    <TableBody>
                      ...
                      <TableRow>
                        <TableCell component="th">Scadenza visita:</TableCell>
                        <TableCell sx={{ 
                          color: isCertificateExpired(selectedIscrizione.dataScadenzaVisita) ? 'error.main' :
                                  isCertificateExpiring(selectedIscrizione.dataScadenzaVisita) ? 'warning.main' : 'inherit',
                          display: 'flex',
                          alignItems: 'center'
                        }}>
                          {formatDate(selectedIscrizione.dataScadenzaVisita)}
                          {isCertificateExpired(selectedIscrizione.dataScadenzaVisita) && (
                            <Tooltip title="Certificato scaduto">
                              <Box component="span" sx={{ 
                                display: 'inline-block', 
                                width: 10, 
                          ...
                          {isCertificateExpiring(selectedIscrizione.dataScadenzaVisita) && (
                            <Tooltip title="Certificato in scadenza entro 30 giorni">
                              <Box component="span" sx={{ 
                                display: 'inline-block', 
                                width: 10, 
